...
- **Navigation**: a fixed sidebar on desktop and a sliding drawer on mobile,
  with a backdrop, `Escape` to close, and automatic close on route change.
...
- **Loading / error / empty states**: every data fetching hook exposes all
  ...
  showing a blank section. Buttons disable themselves during active
  mutations (enroll/remove) to prevent duplicate submissions.
- **Design**: a deliberate warm, dark brown palette (see the `:root`
  ...
  theme, paired with `Fraunces` for headings to avoid the generic blue SaaS
  dashboard look.
...
- Add TanStack Query to get request deduplication, caching, and background
  revalidation for free instead of the current custom hooks.
